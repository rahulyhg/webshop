...
if (isset($_GET['action']) && $_GET['action'] == 'delete') {
    $item_id = $_GET['cid'];
     $is_delete = 1;
     $editQuerydel = "UPDATE `webshop_user` SET `is_deleted` = '" .$is_delete. "' WHERE `id` = '" . $item_id . "'";
//    mysqli_query($con, "delete from  webshop_user where id='" . $item_id . "'");
     mysqli_query($con, $editQuerydel);
    //$_SESSION['msg']=message('deleted successfully',1);
    header('Location:list_blocked_user.php');
    exit();
}
...
if (isset($_GET['action']) && $_GET['action'] == 'block') {
    $item_id = $_GET['cid'];
     $is_delete = 0;
      $editQuerydel = "UPDATE `webshop_user` SET `is_blocked` = '" .$is_delete. "' WHERE `id` = '" . $item_id . "'";
//    mysqli_query($con, "delete from  webshop_user where id='" . $item_id . "'");
     mysqli_query($con, $editQuerydel);
    //$_SESSION['msg']=message('deleted successfully',1);
    header('Location:list_blocked_user.php');
...
<script type="text/javascript">
    function top1(aa)
    {
        location.href = "list_user.php?cid=" + aa + "&action=top1"
    ...
    function del(aa, bb)
    {
        ...
        if (a)
        {
            location.href = "list_blocked_user.php?cid=" + aa + "&action=delete"
        }
    }
    
    function block(aa)
    {
        var a = confirm("Are you sure, you want to Block this Vendor?")
     //alert(aa);
        if (a)
        {
            location.href = "list_blocked_user.php?cid=" + aa + "&action=block"
        }
    }
    ...
    {
        location.href = "list_user.php?cid=" + aa + "&action=disable" + "&name=" + name + "&email=" + email;
    }
    function enable(aa,name, email)
    {
        location.href = "list_user.php?cid=" + aa + "&action=enable" + "&name=" + name + "&email=" + email;
    }
        ...
        <div class="container-fluid">
            ...
            <div class="row-fluid">
                <div class="span12">
                    ...
                    <h3 class="page-title">Blocked User list</h3>
                    <ul class="breadcrumb">
                        <li>
                            <a href="#">Home</a>
                ...
                <div class="span12">
                    ...
                    <div class="widget green">
                        <div class="widget-title">
                            <h4><i class="icon-reorder"></i>List Normal User</h4>
...
if (isset($_GET['action']) && $_GET['action'] == 'enable') {
    $item_id = $_GET['cid'];
    mysqli_query($con, "update webshop_user set is_admin_approved='1',status='1' where id='" . $item_id . "'");
...
$fetch_landlord = mysqli_query($con, "select *,@a:=@a+1 serial_number from webshop_user,(SELECT @a:= 0) AS a WHERE `type`='1' and is_blocked=1 and is_deleted=0 and email_verified=1");
...
if ($num > 0) {
    while ($landlord = mysqli_fetch_array($fetch_landlord)) {
        ...
        ?>
               ...
               <td>
                                            ...
                                            <?php if ($landlord['status'] == '1') { ?>
                                                ...
                                                <?php
                                                echo "<br/>(Check for Certified User)";
                                            } else {
                                            ...
                                            <?php echo stripslashes($landlord['fname'] . " " . $landlord['lname']); ?>
                                                ...
                                                <td>
                                                    <a  href="normaluser_products.php?id=<?php echo $landlord['id'] ?>&action=details">
                                                        List Products</a>
                                                </td>

                                                <td>
                                                    <a  href="view_ownerdetails.php?id=<?php echo $landlord['id'] ?>&action=details">
                                                        View Details</a>
                                                </td>
        ...
        <?php } ?>
                                                ...
                                                <td>
                                                    <a onClick="javascript:block('<?php echo $landlord['id']; ?>')">
                                                     Unblock</a>
                                                </td>
        ...
        <?php
            ...
            ?>
                         ...
                         <!--  <td>
                                                    <?php if ($landlord['enable_disable'] == '0') { ?>
                                <a  onClick="javascript:enable('<?php echo $landlord['id']; ?>');">Click to Enable</a>
                                                    <?php } else { ?>
                                <a  onClick="javascript:disable('<?php echo $landlord['id']; ?>');">Click to Disable</a>
        <?php } ?>
                        </td> -->
                                            </tr>
                                                    <?php
...
function test_mail1($to, $firstname) {
    ...
    $subject = "GMT24- Account deactivation";
