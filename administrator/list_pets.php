...
<script language="javascript">
   function del(aa,bb)
   {
      ...
      if (a)
      {
        location.href="report_user.php?cid="+ aa +"&action=delete"
      }  
   } 
   
function inactive(aa)
   { 
       location.href="report_user.php?cid="+ aa +"&action=inactive"

   } 
   function active(aa)
   {
     location.href="report_user.php?cid="+aa+"&action=active";
   } 
            ...
            <div class="row-fluid">
               <div class="span12">
                  ...
                  <!-- BEGIN PAGE TITLE & BREADCRUMB-->
                   <h3 class="page-title">Pets list</h3>
                   <ul class="breadcrumb">
                       <li>
                           <a href="#">Home</a>
            ...
            <div class="row-fluid">
                <div class="span12">
                    ...
                    <div class="widget green">
                        ...
                        <h4><i class="icon-reorder"></i>List Pets</h4>
...
{
   $item_id=$_GET['cid'];
  mysqli_query($con,"update webshop_reports set status='0' where to_id='".$item_id."'");
...
{
   $item_id=$_GET['cid'];
  mysqli_query($con,"update webshop_reports set status='1' where to_id='".$item_id."'");
  ...
  exit();
                                                        ...
                                                        $fetch_landlord=mysqli_query($con,"select * from webshop_pets order by id desc");
                                                        $num=mysqli_num_rows($fetch_landlord);
                                                        if($num>0)
                                                        {
                                                        while($landlord=mysqli_fetch_array($fetch_landlord))
                                                        {
 $abuseduser_details=mysqli_fetch_array(mysqli_query($con,"select * from webshop_user WHERE id='".$landlord['to_id']."'"));

 $reports_user=mysqli_query($con,"select * from webshop_reports WHERE to_id='".$landlord['to_id']."'");
 
 $total_reports = mysqli_num_rows($reports_user);
                                                           
    if($landlord['image']!='')
    {
    $image_link='../upload/pets_image/'.$landlord['image'];
    }
    else {
    $image_link='../upload/pets_image/nouser.jpg';
    }
              ...
              <tr>
                ...
                <td>
                  ...
                  <td>
                  <?php echo stripslashes($landlord['age']." years");?>
                </td> 
                
                <td>
                  <?php echo stripslashes($landlord['breed']);?>
                </td>

                <td>
                  <?php echo stripslashes($landlord['animal_type']);?>
                </td>
                  ...
                  <td>
                  <?php if($landlord['pets_status'] == '0'){
                    echo "Provider Needed";
                  }else if($landlord['pets_status'] == '1'){
                    echo "Adopted";
                  }else if($landlord['pets_status'] == '2'){
                    echo "Lost";
                  }else if($landlord['pets_status'] == '3'){
                    echo "Found";
                  }
                  ?>
                </td> 
                  ...
                  <td>
                  <?php echo stripslashes($landlord['add_date']);?>
                </td> 
                
                 <td>
                  <a  href="view_owner.php?id=<?php echo $landlord['owner_id'] ?>&action=details">
                   Owner</a>
                  <?php if($landlord['pets_status'] == '1') {?>
                    |<a  href="view_provider.php?id=<?php echo $landlord['provider_id'] ?>&action=details">
                  Provider</a>
                  <?php } ?>
                </td>
               ...
               <!--   <td>
                  <?php if($landlord['status']=='0'){?>
                    <a  onClick="javascript:active('<?php echo $landlord['to_id'];?>');">Click to Unblock</a>
                    <?php } else {?>
                    <a  onClick="javascript:inactive('<?php echo $landlord['to_id'];?>');">Click to Block</a>
                  <?php }?>
                </td> -->
              </tr>
                                                       <?php
