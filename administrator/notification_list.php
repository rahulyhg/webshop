...
if (isset($_GET['action']) && $_GET['action'] == 'delete') {
    $item_id = $_GET['cid'];
    mysqli_query($con, "delete from  webshop_notification where id='" . $item_id . "'");
...
if (isset($_REQUEST['submit'])) {
    ...
    if ($action == 'add' || $action == '') {
        ...
        echo $addQuery = "INSERT INTO `webshop_notification` (`" . implode('`,`', array_keys($fields)) . "`)"
        ...
        $last_id = $_REQUEST['id'];
        $editQuery = "UPDATE `webshop_notification` SET " . implode(', ', $fieldsList)
...
if (isset($_REQUEST['bulk_delete_submit'])) {
    ...
    foreach ($idArr as $id) {
        ...
        mysqli_query($con, "DELETE FROM `webshop_notification` WHERE id=" . $id);
    }
...
if ($_REQUEST['action'] == 'edit') {
    $categoryRowset = mysqli_fetch_array(mysqli_query($con, "SELECT * FROM `webshop_notification`"));
}
?>
...
<script language="javascript">
    ...
    function trend(aa)
    {
        ...
        location.href = "notification_list.php?cid=" + aa + "&action=trend"
    }
    ...
    function del(aa, bb)
    {
        ...
        if (a)
        {
            location.href = "notification_list.php?cid=" + aa + "&action=delete"
        }
    }

    function inactive(aa)
    {
        location.href = "notification_list.php?cid=" + aa + "&action=inactive"

    }
    function active(aa)
    {
        location.href = "notification_list.php?cid=" + aa + "&action=active";
    }
                         ...
                         <!--                 <th>Status</th> -->
                                ...
                                <tbody>
                                    ...
                                    if (isset($_GET['action']) && $_GET['action'] == 'inactive') {
                                        $item_id = $_GET['cid'];
                                        mysqli_query($con, "update webshop_notification set status='0' where id='" . $item_id . "'");
                                        header('Location:notification_list.php');
                                        exit();
                                    }
                                    if (isset($_GET['action']) && $_GET['action'] == 'active') {
                                        $item_id = $_GET['cid'];
                                        mysqli_query($con, "update webshop_notification set status='1' where id='" . $item_id . "'");
                                        header('Location:notification_list.php');
                                        exit();
                                    }
                                    ...
                                    if ($num > 0) {
                                        while ($tools_type = mysqli_fetch_array($fetch_tools_type)) {
                                            ...
                                            $user_noti_details = mysqli_fetch_array(mysqli_query($con, "SELECT * FROM `webshop_user` WHERE `id`='" . $tools_type['from_id'] . "'"));




                                            if ($user_noti_details['image'] != '') {
                                                $image_link = '../upload/user_image/' . $user_noti_details['image'];
                                            } else {
