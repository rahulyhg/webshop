...
if((!isset($_REQUEST['submit'])) && (!isset($_REQUEST['action'])))
{
   ...
   $sql1 = mysqli_query($con,"SELECT DISTINCT user_id FROM makeoffer_product");
...
//echo "<pre>";
//print_r($sarray);
$sellerid = implode(",",$sarray);


$sql = "SELECT * FROM makeoffer_user WHERE id IN ($sellerid)";

  //$sql="select * from makeoffer_user  where ";   
