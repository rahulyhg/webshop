...
{
	$item_id=$_GET['cid'];
	mysqli_query($con,"delete from  webshop_banner where id='".$item_id."'");
...
if((!isset($_REQUEST['submit'])) && (!isset($_REQUEST['action'])))
{
 ...
 $sql="select * from webshop_banner  where id<>''";
...
if(isset($_REQUEST['submit']))
{
  ...
  $fields = array(
   ...
   if($_REQUEST['action']=='edit')
    {     
    $editQuery = "UPDATE `webshop_banner` SET " . implode(', ', $fieldsList)
    ...
    $image =mysqli_query($con,"UPDATE `webshop_banner` SET `image`='".$img_name."' WHERE `id` = '" . mysqli_real_escape_string($con,$_REQUEST['id']) . "'");
    }
...
/* if(isset($_GET['action']) && $_GET['action']=='delete')
{
  ...
  exit();
   ...
   $addQuery = "INSERT INTO `webshop_banner` (`" . implode('`,`', array_keys($fields)) . "`)"
    ...
    $image =mysqli_query($con,"UPDATE `webshop_banner` SET `image`='".$img_name."' WHERE `id` = '" . $last_id . "'");
    }
...
if($_REQUEST['action']=='edit')
{
$categoryRowset = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM `webshop_banner` WHERE `id`='".mysqli_real_escape_string($con,$_REQUEST['id'])."'"));
...
if(isset($_REQUEST['bulk_delete_submit'])){
        ...
        foreach($idArr as $id){
            ...
            mysqli_query($con,"DELETE FROM`webshop_banner` WHERE id=".$id);
        }
...
<script language="javascript">
    ...
    {
        location.href = "list_banner.php?cid=" + aa + "&action=inactive"

    }
    function active(aa)
    {
        location.href = "list_banner.php?cid=" + aa + "&action=active";
    }
