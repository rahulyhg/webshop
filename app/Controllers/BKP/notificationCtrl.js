...
app.controller('notificationCtrl', function ($rootScope, $scope, $http, $location,$timeout, $window, userService, $stateParams) {
...
userService.notificationList(userInfo.user_id).then(function(response) {
		...
		if(response.Ack == '1') {
		$scope.NotificationList=response.NotificationList;		
