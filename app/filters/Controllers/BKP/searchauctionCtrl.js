 ...
 */
app.controller('searchauctionCtrl', function ($rootScope, $scope, $http, $location,$timeout, $window, userService, $stateParams,NgMap) {
...
$scope.searchAuction = function(user) {
 
 //alert('test');
//var userInfo = JSON.parse($window.localStorage["userInfo"]);	
//$scope.user_id=userInfo.user_id;

userService.searchAuction(user,userInfo.user_id).then(function(response) {
		
		$scope.isExists=response.Ack;
              //  alert("aa");
              //  alert(response.Ack);
		if(response.Ack == '1') {
		$scope.all_auction_result=response.all_auction_result;		
		
		} else {
                    $scope.all_auction_result="";
		}
...
userService.searchAuctionall().then(function(response) {
		//alert("bb");
               // alert(response.Ack);
		$scope.isExists=response.Ack;
		if(response.Ack == '1') {
