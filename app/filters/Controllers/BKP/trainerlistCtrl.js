...
app.controller('trainerlistCtrl', function ($rootScope, $scope, $http, $location,$timeout, $window, userService, $stateParams) {
...
userService.trainerList(userInfo.user_id).then(function(response) {
    ...
    console.log('Testing');
		...
		if(response.Ack == '1') {
		$scope.all_trainer=response.all_trainer;
	...
	console.log(err); 
        ...
        $scope.removeTrainer = function(user_id){	
	userService.removeTrainer(user_id).then(function(response) {
