...
if(isset($_GET['action']) && $_GET['action']=='delete')
{
	...
	header('Location:group_post.php');
	exit();
}
...
<script language="javascript">
   function del(aa,bb)
   {
      ...
      if (a)
      {
        location.href="group_post.php?cid="+ aa +"&action=delete"
      }  
   } 
   
function inactive(aa)
   { 
       location.href="group_post.php?cid="+ aa +"&action=inactive"

   } 
   function active(aa)
   {
     location.href="group_post.php?cid="+aa+"&action=active";
   } 
   ...
   <div id="container" class="row-fluid">
    ...
    <?php include("includes/left_sidebar.php"); ?>
      ...
      <div id="main-content">
            ...
            <div class="row-fluid">
               <div class="span12">
                  ...
                  <!-- BEGIN PAGE TITLE & BREADCRUMB-->
                   <h3 class="page-title">Group Post List</h3>
                   <ul class="breadcrumb">
                       <li>
                           <a href="#">Home</a>
        ...
        <tbody>
    <?php
    $fetch_product=mysqli_query($con,"select * from webshop_user  where `product_type`='P'");
    $num=mysqli_num_rows($fetch_product);
    if($num>0)
    {
    while($product=mysqli_fetch_array($fetch_product))
    {
$fetch_store=mysqli_fetch_array(mysqli_query($con,"select * from `webshop_store` where `id`='".$product['store_id']."'"));
$image=mysqli_fetch_array(mysqli_query($con,"SELECT * FROM `webshop_moreimage` WHERE `pro_id`='".$product['id']."'"));
if($image['image']!='')
{
