...
{
	$item_id=$_GET['cid'];
	mysqli_query($con,"delete from  webshop_tbladmin where id='".$item_id."'");
...
if (isset($_REQUEST['submit'])) {
	...
	$queryCat="SELECT id,name from wave_category where id=".$cat_id."";
	...
	$querySubCat="SELECT id,name from wave_subcategory where id=".$subcat."";
	...
	$queryStore="SELECT id,store_title from wave_store where id=".$store_id."";
    ...
    if ($action == 'add' || $action == '') {
       ...
       echo $addQuery = "INSERT INTO `webshop_tbladmin` (`" . implode('`,`', array_keys($fields)) . "`)"
      ...
      $userfile_name = $_FILES['images']['name'][$i];
        ...
        move_uploaded_file($userfile_tmp, $img);
           ...
           $video_link = basename($inv_video);
               ...
               $query_image = "INSERT INTO `wave_moreimage` 
				  (pro_id,image)  
...
{
$categoryRowset = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM `webshop_tbladmin` WHERE `status`='1'"));
...
<script language="javascript">
   function del(aa,bb)
   {
      ...
      if (a)
      {
        location.href="list_admin.php?cid="+ aa +"&action=delete"
      }  
   } 
   
function inactive(aa)
   { 
       location.href="list_admin.php?cid="+ aa +"&action=inactive"

   } 
   function active(aa)
   {
     location.href="list_admin.php?cid="+aa+"&action=active";
   } 
   ...
   <div id="container" class="row-fluid">
    ...
    <?php include("includes/left_sidebar.php"); ?>
      ...
      <div id="main-content">
        ...
        <tbody>
                            <?php
                                                        $fetch_product=mysqli_query($con,"select * from webshop_tbladmin WHERE `status`='1'");
                                                        ...
                                                        {
                                                            //echo "aA".$product['img'];
    //$fetch_store=mysqli_fetch_array(mysqli_query($con,"select * from `webshop_tbladmin` "));
    //$image=mysqli_fetch_array(mysqli_query($con,"SELECT * FROM `webshop_tbladmin`"));
    if($product['img']!='')
    {
    ...
    $image_link='../upload/no.png';
              ...
              <tr>
                ...
                <td>
                  <?php echo stripslashes($product['admin_username']);?>
                </td>
                
                
                
                <td>
                  <?php echo stripslashes($product['txt_pwd']); ?>
                </td>
                
                <td>
                  <?php echo stripslashes($product['email']); ?>
                </td>
